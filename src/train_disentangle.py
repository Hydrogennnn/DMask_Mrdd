...
def valid_by_kmeans(val_dataloader, model, use_ddp, device, noise=False):
    ...
    for Xs, target in val_dataloader:
        ...
        if use_ddp:
            ...
            consist_repr_, vspecific_repr_, concate_repr_, _ = model.all_features(Xs)   # Tensor, list, list
...
def main():
    ...
    for r in range(runtimes):
        ...
        smartprint('Dataset contains {} train samples'.format(len(train_dataset)))
